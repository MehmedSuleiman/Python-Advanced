...
#Worker bees collect nectar. When a worker-bee has found enough nectar, she returns to the hive
# to drop off the load and pass the nectar to waiting bees so they can really start the honey-making process.
...
# total honey waiting for bees made with the collected nectar.
#Step one: check if a bee has collected enough nectar. You should take the first bee and try to
# match it with the last nectar:
#•	If the nectar value is more or equal to the value of the bee, it is considered collected, and the bee returns
# to the hive to drop off the load (step two).
...
#•	If the symbol is "/" and the nectar’s value is 0 (zero), skip the calculation and remove the bee, the
# nectar, and the symbol.
