...
# If there are two or more products with the same name's length, sort them by their name in ascending
# order (alphabetically). In the end, return a string in the following format:
...
def grocery_store(**kwargs):
    sorted_dict = dict(sorted(kwargs.items(), key = lambda item: (-item[1], -len(item[0]), item[0])))
