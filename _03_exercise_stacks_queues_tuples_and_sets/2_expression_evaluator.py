...
# separated with a single space from each other. Your job is to take that string expression and
# calculate the result after evaluating it.
#To do that, you have to follow a simple rule.
# If, for example, we have this string "2 4 * 1 3 -", the first time we meet an operator (*), we
# should take all the numbers we have so far (2, 4), apply that operation to them, and save the result (8).
# The next time we meet an operator (-), we again take all the numbers we have (8, 1, 3) and apply
# the operator to them in that order (8 - 1 - 3 = 4). In the end, we print the result.
